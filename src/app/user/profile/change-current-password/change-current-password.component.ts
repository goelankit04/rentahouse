...
export class ChangeCurrentPasswordComponent implements OnInit {
  @HostBinding('class')
  changeCurrentPasswordComponentClass = 'app-change-current-password';
    ...
    this.form = this._fb.group(
      ...
      },
      {
        validator: [
          ...
          ChangeCurrentPasswordComponent.MatchPassword(
            'new_password',
  ...
  changeCurrentPassword() {
    if (this.user_id) {
      ...
      this._authService.changeCurrentPassword(user).subscribe(
        ...
        (err: any) => {
          // console.log(err);
          this.snackbar.open(err.error.message, '', { duration: 1000 });
        }
      );
    }
  }
