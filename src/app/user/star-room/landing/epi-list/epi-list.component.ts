...
export class EpiListComponent implements OnInit {
  @HostBinding("class") epiListComponentClass = "app-epi-list";
  ...
  ngOnInit() {
    this.router.navigateByUrl(
      `/user/star-room/star-landing/star-episode/${this.first_name}/${this.productId}`
    );
  ...
  openExample(level: number, episode_id: number, id: number) {
  ...
  openPuzzle(level: number, episode_id: number, id: number) {
    level = 13;
    if (this.episodeList[this.getIdIndex(id)].is_available) {
    ...
    this._userService
      ...
      .subscribe(
        ...
        (error: any) => {
          ...
          this._toasterService.pop("errors", error.error.message);
        }
      );
  }
}
